...
const features = [
  ...
  {
    icon: <Repeat className="w-10 h-10 text-accent-400" />,
    title: 'Adaptability',
  ...
  {
    icon: <Target className="w-10 h-10 text-accent-400" />,
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <motion.div
              key={index}
              className="bg-tactical-800 p-8 rounded-lg shadow-md hover:shadow-lg transition-shadow"
            ...
            >
              ...
              <h3 className="font-heading text-xl font-semibold text-tactical-200 mb-3">{feature.title}</h3>
              <p className="text-tactical-300">{feature.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
